...
def compute_port_mapping(offset: int, keep: set[str],
                         cfg: Optional[dict] = None) -> list[dict]:
    """Pure: the host-port mapping for the given offset/keep, no side effects.

    Returns a list of {service, container, protocol, old_host, new_host}. Services
    in `keep` (and offset 0) keep their original host port (new_host == old_host).
    Shared by the overlay generator and the stack_ports tool so what's reported
    always matches what's applied.
    """
    cfg = cfg if cfg is not None else compose_config_json()
    services = cfg.get("services", {})
    mapping: list[dict] = []
    for name in sorted(services):
        shift = 0 if name in keep else offset
        for pt in services[name].get("ports") or []:
            published = pt.get("published")
            target = pt.get("target")
            if published in (None, "") or target is None:
                continue
            old_host = int(published)
            new_host = old_host + shift
            if new_host > 65535:
                raise ValueError(
            ...
            mapping.append({
                "service": name, "container": int(target),
...
def generate_ports_override(offset: int, keep: set[str]) -> tuple[Path, list[dict]]:
    ...
    Returns (overlay_path, mapping). Uses the `!override` YAML tag so the overlay
    REPLACES each service's ports list instead of appending to it (compose merges
    `ports:` by default, which would leave the original port still bound). Services
    in `keep` are left untouched (no override block emitted for them).
    """
    mapping = compute_port_mapping(offset, keep)

    by_service: dict[str, list[dict]] = {}
    for m in mapping:
        if m["new_host"] != m["old_host"]:  # kept/unshifted services need no override
            by_service.setdefault(m["service"], []).append(m)

    blocks: list[str] = []
    for name in sorted(by_service):
        lines = [f"  {name}:", "    ports: !override"]
        for m in by_service[name]:
            lines += [
                f"      - target: {m['container']}",
                f"        published: \"{m['new_host']}\"",
