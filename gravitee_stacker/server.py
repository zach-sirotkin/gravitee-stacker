...
def apim_up(version: str = "latest", variant: str = "default", instance: str = "default",
    ...
    _write_compose_snapshot(apim.project_for(variant, instance),
                            lambda: apim.rendered_compose(instance))
...
def _render_override_yaml(overrides: dict, project: str) -> str:
    def q(v):
        return '"' + str(v).replace("\\", "\\\\").replace('"', '\\"') + '"'
    lines = [
        f"# Rendered gravitee_* OVERRIDES for {project} — the values stacker + your overlays",
        "# actually set (fully interpolated). Edit any value below, then apply with:",
        "#   apim_config(instance=..., action='apply')",
        "# This is the OVERRIDES layer only — the image's gravitee.yml has many more HIDDEN",
        "# defaults (e.g. email.enabled) not shown here. This file is auto-layered onto the",
        "# project on every up/recreate until you apim_config(action='reset').",
        "services:",
    ]
    for svc in sorted(overrides):
        lines.append(f"  {svc}:")
        lines.append("    environment:")
        for k, val in sorted(overrides[svc].items()):
            lines.append(f"      {k}: {q(val)}")
    return "\n".join(lines) + "\n"
...
def compose_snapshot_path(project: str):
    """Where the auto-generated full-compose SNAPSHOT lives for a project. Named as a
    snapshot (not `override`) so it reads as read-only next to the editable override file."""
    return runner.config_dir() / f"{project}.compose-snapshot.yml"


def _write_compose_snapshot(project: str, compose_fn) -> "str | None":
    """Write the full rendered compose for a project as a read-only SNAPSHOT artifact.

    Auto-produced on every *_up and refreshed on config apply/reset so the snapshot always
    matches the live stack (never stale); DELETED on down. It is NOT the config source —
    edits go in <project>.override.yml, which survives version/feature/coexist changes;
    this snapshot is regenerated and would clobber any edits. Best-effort: any failure is
    swallowed so it never blocks an up/down. Set STACKER_COMPOSE_DUMP=0 to disable."""
    if os.environ.get("STACKER_COMPOSE_DUMP", "1").strip().lower() in ("0", "false", "no", "off"):
        return None
    try:
        rendered = compose_fn()
    except Exception:
    ...
    if not rendered:
        return None
    path = compose_snapshot_path(project)
    header = (
        f"# ⚠ READ-ONLY SNAPSHOT — auto-generated by gravitee-stacker for project '{project}'.\n"
        f"# This is a frozen picture of the fully-rendered compose, NOT the config source.\n"
        f"# DO NOT EDIT: it is rewritten on every up and on config apply/reset (your edits\n"
        f"#   would be clobbered) and DELETED on down. If it exists it matches the live stack.\n"
        f"# To CHANGE config, edit the editable diff instead: {project}.override.yml\n"
        f"#   (that survives version/feature/coexist changes), then <stack>_config(action='apply').\n"
        f"# Regenerate this snapshot on demand: <stack>_config(action='show', compose=True).\n"
        f"# Disable auto-generation: export STACKER_COMPOSE_DUMP=0\n\n"
    )
    try:
...
def _remove_compose_snapshot(project: str) -> None:
    """Drop the auto-generated compose snapshot (on down) so it can't go stale. Best-effort."""
    try:
        compose_snapshot_path(project).unlink(missing_ok=True)
    except OSError:
        pass
...
def _stack_config(instance, action, full, *, tool, project, services, override_path,
    ...
    if action == "reset":
        ...
        res = recreate_fn()
        if compose_fn:
            _write_compose_snapshot(project, compose_fn)  # refresh the dump post-reset
    ...
    if action == "apply":
        ...
        res = recreate_fn()
        if compose_fn:
            _write_compose_snapshot(project, compose_fn)  # refresh the dump post-apply
    ...
    if compose and compose_fn:
        ref = _write_compose_snapshot(project, compose_fn)
        out["compose_snapshot_file"] = ref or "could not render compose (is the instance tracked?)"
        if ref:
            out["compose_snapshot_note"] = ("READ-ONLY SNAPSHOT of the FULL rendered compose (every "
                                            "service, image, port, volume, network + all env, fully "
                                            "interpolated). Auto-written on every up, removed on down; "
                                            "compose=True just force-refreshes it. Do NOT edit it — "
                                            "change config via the .override.yml, not here.")
    return out
...
def apim_config(instance: str = "default", action: str = "show", full: bool = False,
    ...
    Read-only reference dumps on `show`: full=True writes each service's in-image gravitee.yml
    (all HIDDEN defaults); compose=True force-refreshes the full-compose SNAPSHOT
    (<project>.compose-snapshot.yml) — which is ALSO auto-written on every up and deleted on
    down. The snapshot is read-only (edits go in the .override.yml); it's the whole stack the
    override diff doesn't repeat, every service/image/port/volume/network + all env, resolved.
    """
...
def apim_down(instance: str = "default", timeout_seconds: int = 180, volumes: bool = False) -> dict:
    ...
    _remove_compose_snapshot(apim.project_for(variant, instance))
    if result["timed_out"]:
...
def am_up(version: str = "latest", instance: str = "default", port: int = 0, features: list = None,
    ...
    _write_compose_snapshot(am.project_for(instance), lambda: am.rendered_compose(instance))
...
def am_down(instance: str = "default", timeout_seconds: int = 180) -> dict:
    ...
    _remove_compose_snapshot(am.project_for(instance))
    if result["timed_out"]:
        return {"status": "timeout", "instance": instance, "message": f"down exceeded {timeout_seconds}s.",
                "stdout_tail": (result["stdout"] or "")[-2000:],
                "stderr_tail": (result["stderr"] or "")[-2000:]}
...
def am_config(instance: str = "default", action: str = "show", full: bool = False,
    ...
    dumps the in-image gravitee.yml (hidden defaults); compose=True refreshes the read-only
    full-compose SNAPSHOT (also auto-written on every up, deleted on down).
    """
    return _stack_config(
...
def _gamma_summarize(instance: str = "default") -> dict:
    ...
    if up.get("tracked") and up.get("running"):
        ...
        overall = "partial"
    offset = up.get("offset") if up.get("offset") is not None else gamma.current_offset(instance)
    return {
        "overall": overall,
        "instance": instance,
        "version": up.get("version") or gamma.current_version(instance),
        "project": gamma.project_for(instance),
        "mode": "coexist" if offset else "canonical",
        "up_process": up,
        "services": [{"service": s, **{k: by.get(s, {}).get(k) for k in ("state", "health", "exit_code")},
                      "label": labels[s]} for s in expected],
...
def gamma_up(version: str = "latest", instance: str = "default", features: list = None,
    ...
    is the only module that wants one; API/AuthZ/Platform Management run without it).

    Instances (generalized coexist): instance="default" uses canonical ports 8082-8086; a named
    instance gets its own project (gravitee-gamma-public-<name>), its own volumes, and an
    auto-allocated host-port band (+20000, +40000) — all five ports and the baked-in console
    URLs shift together. So a named Gamma runs alongside a canonical standalone APIM/AM, or a
    second Gamma VERSION side by side. Gamma bundles APIM, so two stacks on the SAME band collide;
    that's why the second one takes an offset. Because every console is on localhost, coexisting
    consoles SHARE a browser cookie jar — open the second one in Incognito (the result warns).

    Returns immediately — then call `gamma_wait(instance)`. On a port conflict it does NOT start
    (status "port_conflict"); `down_conflicting=true` frees the ports.
    ...
    if err:
        return {"status": "blocked", "message": err}
    if gamma.is_up_running(instance) or gamma.stack_running(instance):
        return {"status": "already_running", "instance": instance, "project": gamma.project_for(instance),
    ...
    if offset:
        warnings.append(
            "MULTIPLE CONSOLES SHARE BROWSER COOKIES: this is a second Gamma platform on localhost, "
            "and browser cookies are scoped by host (not port), so its consoles share one cookie jar "
            "with the other stack's — signing into one logs you out of the other. Open THIS instance's "
            "consoles in an Incognito/private window (or a separate browser profile).")

    conflicts = gamma.detect_conflicts(ports, instance)
    downed = []
    if conflicts:
        if not down_conflicting:
            suggest = {"down_the_other": {"tool": "gamma_up",
                                          "args": {"version": version, "instance": instance, "down_conflicting": True}}}
            if instance == "default":
                suggest["run_another_instance"] = {"tool": "gamma_up", "args": {"version": version, "instance": "b"}}
            return {
                "status": "port_conflict", "version": resolved, "instance": instance, "offset": offset,
                "ports": ports, "conflicts": conflicts,
                "conflicting_projects": sorted({c["project"] for c in conflicts}),
                "message": (f"port(s) {sorted({c['port'] for c in conflicts})} held by "
                            f"{sorted({c['project'] for c in conflicts})}. Gamma bundles APIM, so its ports "
                            "collide with a stack on the same band. down_conflicting=true to free them"
                            + (", or run a named instance to coexist (e.g. instance='b')." if instance == "default" else ".")),
                "suggest": suggest,
            }
        for proj in sorted({c["project"] for c in conflicts}):
    ...
    _write_compose_snapshot(gamma.project_for(instance), lambda: gamma.rendered_compose(instance))
    return {
...
@mcp.tool()
def gamma_status(instance: str = "default") -> dict:
    """Overall verdict + per-service health for a Gamma instance (version, project, URLs)."""
    return _gamma_summarize(instance)


@mcp.tool()
def gamma_list() -> dict:
    """List tracked Gamma instances with their status/version/ports/URLs (for coexist)."""
    out = []
    for inst in gamma.known_instances():
        s = _gamma_summarize(inst)
        out.append({"instance": inst, "overall": s["overall"], "version": s["version"],
                    "project": s["project"], "mode": s["mode"], "urls": s["urls"]})
    return {"status": "ok", "count": len(out), "instances": out}


@mcp.tool()
def gamma_license(instance: str = "default") -> dict:
    """Show the enterprise license entitlements loaded on a running Gamma instance.
    ...
    """
    return runner.read_stack_license(gamma.project_for(instance))


@mcp.tool()
def gamma_wait(instance: str = "default", timeout_seconds: int = 600, poll_seconds: int = 4) -> dict:
    """Block until a Gamma instance is healthy, then return immediately (fails fast on error).
    ...
    """
    return _wait_ready(_gamma_summarize, instance, timeout_seconds, poll_seconds, "gamma")
...
def gamma_down(instance: str = "default", volumes: bool = False, timeout_seconds: int = 180) -> dict:
    ...
    _remove_compose_snapshot(gamma.project_for(instance))
    ...
    return {"status": status, "instance": instance, "volumes_removed": volumes,
            "returncode": res.get("returncode"), "stdout": res.get("stdout", ""), "stderr": res.get("stderr", "")}
...
def gamma_config(instance: str = "default", action: str = "show", full: bool = False,
    ...
    full=True dumps the in-image gravitee.yml (hidden defaults); compose=True refreshes the read-only
    full-compose SNAPSHOT (also auto-written on every up, deleted on down).
    """
    return _stack_config(
