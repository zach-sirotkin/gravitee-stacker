...
def apim_up(version: str = "latest", variant: str = "default", instance: str = "default",
    ...
    _write_rendered_compose(apim.project_for(variant, instance),
...
def rendered_compose_path(project: str):
    """Where the auto-generated full rendered compose lives for a project."""
    return runner.config_dir() / f"{project}.rendered-compose.yml"


def _write_rendered_compose(project: str, compose_fn) -> "str | None":
    """Write the full rendered compose for a project as a lifecycle artifact.

    Called automatically on every *_up and on config apply/reset so the file always
    matches the live stack (never stale). Best-effort: any failure is swallowed so it
    never blocks an up/down. Set STACKER_COMPOSE_DUMP=0 to disable auto-generation."""
    ...
    if not rendered:
        return None
    path = rendered_compose_path(project)
    header = (
        f"# AUTO-GENERATED by gravitee-stacker for project '{project}'.\n"
        f"# Written on every up and refreshed on config apply/reset; DELETED on down —\n"
        f"# so if this file exists it reflects the currently-tracked stack (not stale).\n"
        f"# Read-only reference. Edit config via the <project>.override.yml, not here.\n"
        f"# Regenerate on demand: <stack>_config(action='show', compose=True).\n"
...
def _remove_rendered_compose(project: str) -> None:
    """Drop the auto-generated rendered compose (on down) so it can't go stale. Best-effort."""
    try:
        rendered_compose_path(project).unlink(missing_ok=True)
    except OSError:
        pass
...
def _stack_config(instance, action, full, *, tool, project, services, override_path,
    ...
    if action == "reset":
        ...
        res = recreate_fn()
        if compose_fn:
            _write_rendered_compose(project, compose_fn)  # refresh the dump post-reset
    ...
    if action == "apply":
        ...
        res = recreate_fn()
        if compose_fn:
            _write_rendered_compose(project, compose_fn)  # refresh the dump post-apply
    ...
    if compose and compose_fn:
        ref = _write_rendered_compose(project, compose_fn)
        out["rendered_compose_file"] = ref or "could not render compose (is the instance tracked?)"
        if ref:
            out["rendered_compose_note"] = ("The FULL rendered compose (every service, image, port, "
                                            "volume, network + all env, fully interpolated). This is "
                                            "auto-written on every up and removed on down; compose=True "
                                            "just force-refreshes it now.")
    return out
...
def apim_down(instance: str = "default", timeout_seconds: int = 180, volumes: bool = False) -> dict:
    ...
    _remove_rendered_compose(apim.project_for(variant, instance))
    if result["timed_out"]:
...
def am_up(version: str = "latest", instance: str = "default", port: int = 0, features: list = None,
    ...
    _write_rendered_compose(am.project_for(instance), lambda: am.rendered_compose(instance))
...
def am_down(instance: str = "default", timeout_seconds: int = 180) -> dict:
    ...
    _remove_rendered_compose(am.project_for(instance))
    if result["timed_out"]:
        return {"status": "timeout", "instance": instance, "message": f"down exceeded {timeout_seconds}s.",
                "stdout_tail": (result["stdout"] or "")[-2000:],
                "stderr_tail": (result["stderr"] or "")[-2000:]}
...
def gamma_up(version: str = "latest", instance: str = "default", features: list = None,
    ...
    _write_rendered_compose(gamma.project_for(instance), lambda: gamma.rendered_compose(instance))
    return {
...
def gamma_down(instance: str = "default", volumes: bool = False, timeout_seconds: int = 180) -> dict:
    ...
    _remove_rendered_compose(gamma.project_for(instance))
