...
### Added
- **Config view/tweak on all three stacks — `apim_config` / `am_config` / `gamma_config`.**
  Surfaces the `gravitee_*` overrides stacker + overlays actually set (fully interpolated) —
  the top overrides layer, not the image's hidden gravitee.yml defaults — to an editable
  `~/.gravitee/stacker-config/<project>.override.yml`. `action="show"` (re)writes it (with
  `full=True` also dumping each service's in-image gravitee.yml so hidden defaults are
  visible); `action="apply"` recreates the config services so edits take effect (volumes
  ...
  startup-time, so "on the fly" means edit → recreate (~20s), not hot-reload. Verified live
  end-to-end on APIM and AM; Gamma validated (show + override merge; apply is the shared path).
